...
class ACPCareerScraper(BaseScraper):
    name = "ACP Career Connection"
    source_type = "job_board"
    rate_limit = 4.0
    base_url = "https://careers.acponline.org"

    def scrape(self, state: str, terms: List[str]) -> List[Job]:
        jobs = []
        searches = ["internal+medicine", "hospitalist", "general+internist"]
        for term in searches:
            url = f"{self.base_url}/jobs?q={term}&location={state}"
    ...
    def _scrape_page(self, url: str, state: str) -> List[Job]:
        ...
        try:
            ...
            listings = soup.select(".job, .job-listing, .listing, article")
            for item in listings:
                title_el = item.select_one("h2, h3, .title, .job-title, a")
                employer_el = item.select_one(".employer, .company, .organization")
            ...
            log.warning(f"ACP Career blocked: {e}")
        except Exception as e:
            log.error(f"ACP Career error {url}: {e}")
        return jobs
