...
class JAMACareerScraper(BaseScraper):
    ...
    rate_limit = 4.0
    base_url = "https://careers.jamanetwork.com"

    def scrape(self, state: str, terms: List[str]) -> List[Job]:
        jobs = []
        state_param = STATE_FULL.get(state, state)
        searches = ["internal+medicine", "hospitalist", "primary+care"]
        for term in searches:
            url = f"{self.base_url}/jobs?q={term}&location={state_param}"
    ...
    def _scrape_page(self, url: str, state: str) -> List[Job]:
        ...
        try:
            ...
            listings = soup.select(".job, .job-listing, .result-item, article")
            ...
            log.warning(f"JAMA blocked: {e}")
        except Exception as e:
            log.error(f"JAMA error {url}: {e}")
        return jobs
